...
def plot_mmpbsa(mmpbsa_df, mmpbsa_summary, outdir: Path):
    ...
    if not delta.empty:
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(SYSTEMS, rotation=25, ha="right")
        ax.set_ylabel("MM/GBSA ΔGbind (kcal/mol)")
...
def write_methods_and_response(outdir: Path, rep_summary, sys_summary, mmpbsa_summary):
    ...
    if not delta_rows.empty:
        for sys in SYSTEMS:
            ...
            if not row.empty:
                delta_text.append(
                    f"{sys}: {float(row['replicate_mean_kcal_per_mol'].iloc[0]):.2f} ± "
                    f"{float(row['replicate_sd_kcal_per_mol'].iloc[0]):.2f} kcal/mol"
                )
    delta_sentence = "; ".join(delta_text) if delta_text else "MM/GBSA ΔTOTAL values are summarized in the generated table."
