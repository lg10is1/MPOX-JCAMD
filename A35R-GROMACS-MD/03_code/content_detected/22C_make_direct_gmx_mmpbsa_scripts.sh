...
# 5) Last wider search under <SOFTWARE_ROOT>, restricted by gmx/mmpbsa keywords
if [[ -z "$SIF" ]]; then
  SIF="$(find <SOFTWARE_ROOT> -type f \( -name "*.sif" -o -name "*.simg" -o -name "*.img" \) 2>/dev/null | grep -Ei 'mmpbsa|gmx' | head -n 1 || true)"
fi
...
if [[ -z "$SIF" ]]; then
  ...
  echo "       grep -n . $WRAP"
  echo "       find <SOFTWARE_ROOT> -type f -name '*.sif' 2>/dev/null | grep -Ei 'mmpbsa|gmx'"
  exit 1
fi
