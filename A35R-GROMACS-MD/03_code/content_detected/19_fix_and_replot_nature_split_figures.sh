...
OUTDIR="gmx_nature_split_figures_v2"

echo "============================================================"
echo "[INFO] Replot Nature-style split figures with automatic time-unit detection"
echo "[INFO] BASE=$BASE"
...
try:
    ...
    print("[ERROR] matplotlib unavailable:", e)
    raise SystemExit(1)
PY

cat > 19_plot_nature_split_gromacs_fixed.py <<'PY'
#!/usr/bin/env python3
import argparse
import csv
import math
...
chmod +x 19_plot_nature_split_gromacs_fixed.py

python3 19_plot_nature_split_gromacs_fixed.py \
  --root gmx_md100_3rep \
  --out "$OUTDIR" \
  --smooth-window 101
...
echo
echo "============================================================"
echo "[INFO] Check fixed figures"
echo "============================================================"

echo "[INFO] PDF examples:"
find "$OUTDIR/per_system_three_reps_panel" -name "*.pdf" | head -n 20

echo
echo "[INFO] Time unit modes:"
cut -d, -f1,2,3,5,6,7 "$OUTDIR/per_replicate_summary_statistics.csv" | head -n 30

echo
echo "[INFO] File counts:"
echo -n "PNG: "; find "$OUTDIR" -name "*.png" | wc -l
echo -n "PDF: "; find "$OUTDIR" -name "*.pdf" | wc -l
echo -n "SVG: "; find "$OUTDIR" -name "*.svg" | wc -l

echo
echo "[DONE] Fixed Nature-style figures are in: $OUTDIR"
