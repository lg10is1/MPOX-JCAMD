...
def normalize_line(raw: str) -> str:
    s = raw.strip()
    s = re.sub(r"^\d+:\s*", "", s)      # 鍏煎 grep -n 杈撳嚭
    s = s.replace("螖", "DELTA_")        # 螖TOTAL -> DELTA_TOTAL
    s = s.replace("|", " ")
...
def parse_delta_section(dat_file: Path):
    """
    鍙В鏋?Delta (Complex - Receptor - Ligand) 涓嬮潰鐨?螖 椤广€?    涓嶈В鏋?Complex/Receptor/Ligand 鍚勮嚜鐨?TOTAL锛岄伩鍏嶆妸缁濆鑳介噺璇涓虹粨鍚堣嚜鐢辫兘銆?    """
    results = {}
    ...
    for raw in lines:
        ...
        # gmx_MMPBSA 杈撳嚭鍒椾竴鑸槸锛?        # Average SD(Prop.) SD SEM(Prop.) SEM
        if len(nums) < 1:
            continue
...
def main():
    ...
    # wide 琛ㄥ瓧娈佃嚜鍔ㄦ敹闆?    wide_fields = ["system", "rep", "source_file"]
    ...
    print()
    print("============================================================")
    print("MM/GBSA 螖TOTAL summary")
    print("============================================================")

    if not delta_total_rows:
        print("[ERROR] Still no DELTA_TOTAL parsed.")
        print("[HINT] Please check whether FINAL_RESULTS_MMPBSA_GB.dat contains 螖TOTAL.")
    else:
