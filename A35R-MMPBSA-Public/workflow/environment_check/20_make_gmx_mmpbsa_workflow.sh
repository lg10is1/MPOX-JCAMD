...
To further refine the docking-derived binding poses and quantitatively assess the relative binding stability of the A35R鈥搇igand complexes, MM/GBSA and MM/PBSA calculations were performed using gmx_MMPBSA. For each ligand, three independent 100-ns GROMACS trajectories were used. The last 50 ns of each trajectory were extracted at 500-ps intervals, giving approximately 101 snapshots per replicate and approximately 303 snapshots per ligand. Prior to energy calculation, trajectories were processed with GROMACS trjconv to remove periodic boundary artifacts, center the protein鈥搇igand complex, and fit the trajectory to the protein鈥搇igand complex. The single-trajectory protocol was used, in which receptor and ligand conformations were extracted from the same complex trajectory. The receptor group was defined as the protein atoms and the ligand group as the LIG residue. The GROMACS topology generated from the Amber/GAFF2-parametrized system was provided directly to gmx_MMPBSA, thereby retaining the ligand parameters and partial charges used in the MD simulations. The generalized Born model was calculated with igb = 5 and a salt concentration of 0.150 M. In additional validation runs, Poisson鈥揃oltzmann calculations were performed with an ionic strength of 0.150 M. The binding free energy was estimated as 螖Gbind = Gcomplex 鈭?Greceptor 鈭?Gligand. Energy components, including van der Waals, electrostatic, polar solvation, and nonpolar solvation contributions, were extracted for each system. Residue-wise energy decomposition was further performed to identify residues that contributed most strongly to ligand binding.

Result-description template

Compared with the initial docking score-based ranking, the MM/GBSA/MM/PBSA analysis provided a trajectory-ensemble-based estimate of binding stability after explicit-solvent MD refinement. A more negative 螖Gbind indicates a more favorable calculated binding free energy under the same force-field and implicit-solvent settings. The decomposition of 螖Gbind into van der Waals, electrostatic, polar solvation, and nonpolar solvation terms allows the dominant energetic drivers of binding to be assessed. In this study, the three A35R鈥搇igand systems were evaluated using three independent 100-ns simulations, thereby reducing the dependence on a single docking pose or a single MD trajectory. Residue-wise decomposition was used to identify binding-site residues with favorable energetic contributions and to support the detailed interaction analysis of the predicted binding poses.
EOS
